...
onMounted(() => {
  instance.root.appContext.components('aaa', PLUS_RE)
})
defineProps({
  title: {
    type: String,
...
<template>
  <div class="story">
    <div class="bar">
      <h2>{{ title }}</h2>
      <button aria-pressed="showCode" @click="handleToggleShowCode">
        Show code
      </button>
    </div>
...
.code {
//position: absolute; //top: 0; //left: 0; //width: 100%; //height: 100%; //padding: 0 0.5rem; //background-color: #1e1e1e; //overflow: auto; //text-align: left; //font-size: 0.8rem;
}
</style>
